...
/**
 * 由 fuelphp 的 format 類別，修正成適用 kohana 的 format 類別
 * ※ fuelphp 原始的 to_cvs有問題！！因為幾乎不會用到，懶的去回報修正了
 *
...
class Format
{
    ...
    {
        self::$config = Kohana::$config->load('format')->as_array();
        self::$config = is_array(self::$config) ? self::$config : array();
    ...
    public function __construct($data = null, $from_type = null)
    {
        ...
        if ($from_type !== null) {
            if (method_exists($this, '_from_' . $from_type)) {
                ...
                throw new Kohana_Exception('Format class does not support conversion from :from_type', array(':from_type' => $from_type));
            }
        }
        $this->_data = $data;
    }
    ...
    public function to_xml($data = null, $structure = null, $basenode = 'xml')
    {
        ...
        foreach ($data as $key => $value) {
            ...
            if (is_numeric($key)) {
                // make string key...
                $key = (Inflector::singular($basenode) != $basenode) ? Inflector::singular($basenode) : 'item';
            }
    ...
    {
        // csv format settings
        $newline = Arr::get(self::$config, 'format.csv.newline', "\n");
        $delimiter OR $delimiter = Arr::get(self::$config, 'format.csv.delimiter', ',');
        $enclosure = Arr::get(self::$config, 'format.csv.enclosure', '"');
        $escape = Arr::get(self::$config, 'format.csv.escape', '\\');
        ...
        if (is_array($data) AND Arr::is_multi($data)) {
            ...
            if (is_array($data[0]) AND Arr::is_assoc($data[0])) {
    ...
    {
        $callback OR $callback = Arr::get($_REQUEST, 'callback');
    ...
    public function to_yaml($data = null)
    {
        ...
        if (!function_exists('spyc_load')) {
            include Kohana::find_file('vendor', 'spyc/spyc', 'php');
        }
    ...
    protected function _from_csv($string)
    {
        ...
        $rows = preg_split('/(?<=' . preg_quote(Arr::get(self::$config, 'format.csv.enclosure', '"')) . ')' . Arr::get(self::$config, 'format.csv.regex_newline', '\n') . '/', trim($string));

        // csv config
        $delimiter = Arr::get(self::$config, 'format.csv.delimiter', ',');
        $enclosure = Arr::get(self::$config, 'format.csv.enclosure', '"');
        $escape = Arr::get(self::$config, 'format.csv.escape', '\\');
